...
# # 콜론 : 코드 볼록의 시작을 나타내는 것으로 콜론 이후부터가 실행될 문장이다.
...
# if selectedNumber == KOREA_NUMBER: # 1 (x)
#     print('주문하시겠습니까?')
# elif selectedNumber == USA_NUMBER:  # 2 (x)
#     print('Would you like to order?')
# elif selectedNumber == CHINA_NUMBER:  # 3 (x)
#     print('请问您要点餐吗？')
# else: 
#     print('Would you like to order?')
...
# 오늘 날짜를 구한다.
# 고농도 미세먼지 비상저감조치를 위한 차량 2부제 프로그램입니다.
# 다음 요구사항과 결과 화면을 참고하여 프로그램을 완성해보자.
# from datetime import datetime
...
# lifeTime = int(input('최초 장비를 사용하기까지 걸린시간(초)을 입력하세요. '))
...
# 삼항 연산자(조건식)와 if~else문을 이용해서 프로그램을 만드시오.
# 시험점수를 입력한다
...
# or \
# (ranNum == 2 and myNum ==2):
# print('컴퓨터 승')

# quiz)
# '''
# 사용자가 입력한 문자 메시지 길이에 따라서 SMS 또는 MMS의 발송을 결정하는 프로그램을 완성하시
# 오(단, 메시지 길이가 50 이하이면 SMS 발송, 그렇지 않으면 MMS를 발송한다.)
# '''
